package kr.hhplus.be.server.api.service.reservation.presentation;
...
import kr.hhplus.be.server.api.service.reservation.presentation.dto.ReservationRequest;
import kr.hhplus.be.server.api.service.reservation.application.ReservationFacade;
import kr.hhplus.be.server.api.service.reservation.presentation.dto.ReservationResponse;
...
public class ReservationController {
	...
	@PostMapping()
	public ResponseEntity<ApiResult<ReservationResponse>> reserveSeat(
		...
		return ResponseEntity.ok(ApiResult.of("좌석이 예약되었습니다.", response));
	}
	...
	@PostMapping("/payment")
	public ResponseEntity<ApiResult<ReservationResponse>> payForSeat(
		@Valid @RequestBody ReservationRequest req) {
		ReservationResult reservation = reservationFacade.payAmountAndConfirmReservation(
			req.toDto());
		ReservationResponse response = ReservationResponse.from(reservation);
		return ResponseEntity.ok(ApiResult.of("좌석이 결제되었습니다.", response));
	}
